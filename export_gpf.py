...
class GPFExportDialog(pya.QDialog):
    ...
    def handle_export(self) -> None:
        ...
        freebeam_bin = self._find_freebeam_binary()

        if not freebeam_bin:
            pya.QMessageBox.critical(
                self,
                "Freebeam not found",
                "The Freebeam 'gpfout' binary is required. Install it with install.sh "
                "or set FREEBEAM_BIN to an existing executable before exporting.",
            )
            return

        if source_filename and all_layers_selected and not any_heal_requested:
            input_gds = source_filename
            source_for_name = source_cell.name
        else:
            with tempfile.NamedTemporaryFile(suffix=".gds", delete=False) as tmp:
                filtered_layout.write(tmp.name)
                tmp.flush()
                input_gds = tmp.name
                source_for_name = top_cell.name

        try:
            self._export_with_freebeam_cli(
                freebeam_bin,
                gpf_path,
                exported_at,
                layer_metadata,
            ...
            if input_gds and input_gds != source_filename and Path(input_gds).exists():
                Path(input_gds).unlink(missing_ok=True)

        pya.QMessageBox.information(
            self,
            "Export complete",
            f"Saved GPF with Freebeam to: {gpf_path}\nLayers: {len(selected)}",
        )
    ...
    def _find_freebeam_binary(self) -> str | None:
        """Locate a usable Freebeam ``gpfout`` binary.

        The search order is:
        1. ``FREEBEAM_BIN`` environment variable.
        2. A ``gpfout`` executable on the PATH.

        Returns the resolved path or ``None`` when unavailable.
        """

        from os import environ

        configured = environ.get("FREEBEAM_BIN")
        if configured and Path(configured).exists():
            return str(Path(configured).resolve())

        found = shutil.which("gpfout")
        return found

    def _export_with_freebeam_cli(
        self,
        freebeam_bin: str,
        gpf_path: str,
        exported_at: str,
        ...
        """Delegate conversion to the Freebeam command-line tool.

        Notes:
            Freebeam expects explicit write window extents and beam settings.
            We derive a conservative bounding box from the filtered GDS and
            provide a relative-dose file for the selected layers.
        """
        ...
        block_size_x = max(urx - llx, 1.0)
        block_size_y = max(ury - lly, 1.0)

        with tempfile.NamedTemporaryFile("w", delete=False) as dose_file:
            for layer in layer_metadata:
                info: pya.LayerInfo = layer["info"]
                dose_file.write(f"{info.layer} {info.datatype} {layer['relative_dose']}\n")
            dose_path = dose_file.name
        ...
        cmd = [
            ...
            f"{block_size_x}",
            f"{block_size_y}",
            "0",  # let Freebeam pick subfield size
            "0",
            "5",  # beam_step_size_nm
            "1",  # main_field_lsb_um
            "100",  # voltage_kv
        ...
        Path(dose_path).unlink(missing_ok=True)
        if result.returncode != 0:
            raise RuntimeError(
                f"Freebeam gpfout failed with code {result.returncode}: {result.stderr}"
            )
